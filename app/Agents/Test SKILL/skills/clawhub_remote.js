...
export const skill = {
    ...
    description: "ClawHub.ai uzak ekosisteminde yetenek (skill) arar, inceler ve SKILL.md dosyalarini indirir. Yeni yetenek kesfetmek icin bu araci kullan. Arama: action='search', query='anahtar kelime'. Detay: action='inspect', query='skill-slug'. Indirme: action='download', query='skill-slug'.",
    parameters: {
        type: "object",
        properties: {
            action: {
                ...
                description: "'search' ClawHub.ai'de vektör arama yapar, 'inspect' skill detaylarını getirir, 'download' SKILL.md'yi yerel Marketplace'e indirir"
            },
            query: {
                type: "string",
                description: "Arama terimi veya skill slug'ı (ör: 'pdf', 'github-sync', 'web-scraper')"
            }
        },
...
Kurmak için indirdikten sonra: clawhub_install(action: "install", skill_name: "${slug}")`;
            ...
            if (action === "download") {
                ...
                const downloadUrl = `${CLAWHUB_BASE}/api/v1/download?slug=${encodeURIComponent(slug)}&version=latest`;
                ...
                try {
                    ...
                    if (dlResp.ok) {
                        ...
                        if (contentType.includes('zip') || contentType.includes('octet-stream')) {
                            ...
                            return `[CLAWHUB İNDİRME BAŞARILI] '${slug}' ZIP olarak indirildi.
Konum: ${zipPath}
Boyut: ${(buffer.length / 1024).toFixed(1)} KB

Not: Bu ClawHub skill'i SKILL.md formatındadır (AgentsHUB .js formatından farklı).
ZIP içeriğindeki SKILL.md dosyasını okumak için byterover ile arşivi açabilirsin.
Veya clawhub_install ile yerel Marketplace'ten kurulabilir.`;
                        } else {
                            // Text/JSON olabilir
