...
const paramDescriptions = {
    // web / internet
    url: "Ziyaret edilecek veya kontrol edilecek web sitesi adresi (örn: 'https://example.com/')",
    ...
    extract_text: "Sayfa yüklendiğinde metnin çekilip çekilmeyeceği (true/false) - Genelde true gönderilmelidir",
...
function fixSchemas() {
    ...
    for (let f of files) {
        ...
        for (let i = 0; i < lines.length; i++) {
            // Emojilerle baslayan description bloklari bozuk oldugunu gosteriyor.
            // RegExp matcher: /(🧠|🦁|🌐|💻|🧮|📦|📋|🦆|📧|🕐|🐙|🔍|☁️|🛡️|🌉|📄|🐍|📸|📡|✨|📊|🌤️|🔗|💾)/
            if (lines[i].includes('description:') && lines[i].match(/(🧠|🦁|🌐|💻|🧮|📦|📋|🦆|📧|🕐|🐙|🔍|☁️|🛡️|🌉|📄|🐍|📸|📡|✨|📊|🌤️|🔗|💾)/)) {
                
                // Ana modül description u zaten doqru. Schema properties icinde miyiz? (iceride girinti olmalı)
                if (lines[i].includes('type:') || lines[i].match(/^\s{6,}/)) {
                    ...
                    if (!paramName) {
                       for (let j = 1; j <= 4; j++) {
                           if (i-j >= 0) {
                               ...
                               if (upMatch && !lines[i-j].includes('properties:') && !lines[i-j].includes('parameters:')) {
                                   paramName = upMatch[1];
                                   break;
                               }
                           }
                    ...
                    if (paramName && paramDescriptions[paramName]) {
                        ...
                        modified = true;
                    } else if (paramName && paramName !== 'description') {
